...
def zhuan(i,a,b,L,cv):   #改变i的方向
    if budongle(i,a,b,L,cv):
        return -a,-b
    else:
        if b == 0 :
            x=random()
            if Up(i,L,cv) and Down(i,L,cv):
                if x > 0.75:
                    a,b = 0,-1
                elif x > 0.5:
                    a,b = 0,1
            ...
            y=random()
            if Right(i,L,cv) and Left(i,L,cv):    
                if y > 0.75:
                    a,b = -1,0
                elif y > 0.5:
                    a,b = 1,0
...
def movetriangle(event):   #第一关移动吃豆人
    global c
    global d
    if event.keysym == 'Up':
        c,d=0,-1        
    elif event.keysym == 'Down':#Down,Left,Right就是方向不一样，其他的原理都一样。就不多加赘述了。
        c,d=0,1         
...
def eat(cv,i,L,beans,windows):
    global life
    global score
    positionOfEater=[97,97,97,68,97]
    xPositionOfDemons=[355,415,355,275,215]
    itemsOfWallsAndEater=[9,16,16,13,26]
    itemsBesidesBeans=[13,20,20,17,30]
...
def paintCanvas1(): #LEVEL1
    ...
    bigbean1=bg1.create_oval(346,237,366,253,outline='GreenYellow',fill='GreenYellow') # 放置两个大豆子
    bigbean2=bg1.create_oval(230,467,250,483,outline='GreenYellow',fill='GreenYellow')

    bean=list(bg1.find_overlapping(0, 0, 600, 600))
    bean1=bean[0:len(bean)]

    deleteBeans("map1.txt",bg1,1)
    ...
    while True:
        ...
        if final[-1] == itemsBesidesBeans[0] :
...
def paintCanvas2():#LEVEL2
    ...
    ol2 = bg2.create_rectangle(p2, outline='', fill="LightSkyBlue")
    ...
    bigbean1=bg2.create_oval(346,237,366,253,outline='GreenYellow',fill='GreenYellow') # 放置两个大豆子
    bigbean2=bg2.create_oval(230,467,250,483,outline='GreenYellow',fill='GreenYellow')
...
def paintCanvas3():#LEVEL3
    ...
    bigbean1=bg3.create_oval(205,435,225,451,outline='GreenYellow',fill='GreenYellow') # 放置两个大豆子
    bigbean2=bg3.create_oval(495,205,515,221,outline='GreenYellow',fill='GreenYellow')
...
def paintCanvas4(): #LEVEL4
    ...
    bigbean1=bg4.create_oval(465,520,485,536,outline='GreenYellow',fill='GreenYellow') # 放置两个大豆子
    bigbean2=bg4.create_oval(345,205,365,221,outline='GreenYellow',fill='GreenYellow')
...
def paintCanvas5(): #LEVEL5
    ...
    bigbean1=bg5.create_oval(145,300,165,316,outline='GreenYellow',fill='GreenYellow') # 放置两个大豆子
    bigbean2=bg5.create_oval(405,300,425,316,outline='GreenYellow',fill='GreenYellow')
...
def Invincible():  # 第一关
    ...
    a20 = cvI.create_image(475, 474, image=dem1, anchor=CENTER)  # 放置怪物的初始位置
    a21 = cvI.create_image(475, 474, image=dem1, anchor=CENTER)  # 放置怪物的初始位置
    a22 = cvI.create_image(475, 474, image=dem1, anchor=CENTER)  # 放置怪物的初始位置
    a23 = cvI.create_image(475, 474, image=dem1, anchor=CENTER)  # 放置怪物的初始位置
    ...
    f = open("map1.txt", "r")  # 读入地图中的矩形的大致
    r = f.readlines()  # 全部读取
    list1 = []  # 创建用来存储坐标的list1
    for i in r:
        aII = i.split()  # 将r拆分为坐标数值的列表
        for t in aII:  # 遍历列表
            t = int(t)  # 将列表中的元素类别转化为数值
            s = (t + 5) / 10 * 10  # 四舍五入精确坐标数值
            list1.append(s)  # 添加到list1中

    listx1 = []  # 坐标分类 因为此时list1中的坐标数值是以' x1,x2,y1,y2 ,x1,x2,y1,y2 …… '的形式存在的，所以在这里将其分类到专门存储x1,x2,y1,y2的四个列表中
    listy1 = []
    listx2 = []
    listy2 = []
    for i in range(0, len(list1), 4):  # 每四个为一组
    ...
    for i in range(len(listx1)):  # 遍历坐标列表，四个一组为一个地图的矩形坐标
        ...
        x2 = listx2[i]
        y2 = listy2[i]
        list3 = list(cvI.find_overlapping(x1, y1, x2, y2))  # 找出与在地图矩形内的豆豆
    ...
    Label(winI, text="Your lives: +∞", fg='black', font="Times -15 bold",
          bg='LightSkyBlue').place(x=300, y=600, anchor=S)
    while True:
        cvI.bind_all('<KeyPress-Up>', movetriangleI)  # 绑定键盘的上下左右键
        cvI.bind_all('<KeyPress-Down>', movetriangleI)
        cvI.bind_all('<KeyPress-Left>', movetriangleI)
        cvI.bind_all('<KeyPress-Right>', movetriangleI)
        ...
        zhq1(a20, x0, y0)
        x0, y0 = zhuanI(a20, x0, y0)
        zhq1(a21, x1, y1)
        x1, y1 = zhuanI(a21, x1, y1)
        zhq1(a22, x2, y2)
        x2, y2 = zhuanI(a22, x2, y2)
        zhq1(a23, x3, y3)
        x3, y3 = zhuanI(a23, x3, y3)
        cvI.update()
...
def movetriangleI(event):  # 更改行走方向
    global cI
    global dI
    if event.keysym == 'Up':
        cI, dI = 0, -1
...
def UpI(i):
    xyb2 = list(cvI.coords(i))
    xyb_ = list(cvI.find_overlapping(xyb2[0] - 14, xyb2[1] - 14 - 10, xyb2[0] + 14, xyb2[1] + 14 - 10))
    for n in xyb_:
        if n < 9:
            return False
    return True
...
def RightI(i):
    xyb2 = list(cvI.coords(i))
    xyb_ = list(cvI.find_overlapping(xyb2[0] - 14 + 10, xyb2[1] - 14, xyb2[0] + 14 + 10, xyb2[1] + 14))
    for n in xyb_:
        if n < 9:
            return False
    return True
...
def zhq1(i, aII, bI):  # 不进墙
    xyb2 = list(cvI.coords(i))
    xyb_ = list(cvI.find_overlapping(xyb2[0] - 14 + aII, xyb2[1] - 14 + bI, xyb2[0] + 14 + aII, xyb2[1] + 14 + bI))
    if xyb_[0] < 9:
        cvI.move(i, 0, 0)
    else:
        cvI.move(i, aII, bI)
...
def zhq2(cI, dI):  # 吃豆人
    xyc = list(cvI.coords(aI))
    xyc_ = list(cvI.find_overlapping(xyc[0] - 8 + cI, xyc[1] - 8 + dI, xyc[0] + 8 + cI, xyc[1] + 8 + dI))
    if xyc_[0] < 9:
        cvI.move(aI, 0, 0)
    else:
        cvI.move(aI, cI, dI)
...
def Help(): #帮助界面
    ...
    helptext = "利用键盘方向键控制吃豆人的移动方向，避开怪物，把地图上的所有豆豆吃掉过关！\n\n" \
               ...
               "并且增加一条性命\n怪物：怪物会不断靠近你，一旦碰到怪物会失去一条性命" \
               "\n\n游戏共有5关，在每一关你有初始的三条性命，" \
               "性命全部失去后会GAME OVER\n\n\n\n" \
